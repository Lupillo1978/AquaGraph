...
export default class DietEngine {
    ...
    registerEvents() {
        ...
        document.getElementById("btnSaveDiet").addEventListener("click", async () => {
            ...
            try {
                const response = await this.controller.create(diet);
        ...
        // Agregar Bloque
        // ==========================
        document.getElementById("btnAddDietItem").addEventListener("click", () => {
            this.addItem();
        });
        ...
        // Cancelar
        // ==========================
        document.getElementById("btnCancelDiet").addEventListener("click", () => {
            if (this.manager) {
                this.manager.show();
            }
        });
    ...
    renderItems() {
        ...
        this.items.forEach((item, index) => {
            const row = new DietItemRow(item, index);
    ...
    registerItemEvents() {
        document.querySelectorAll("#dietItems tr").forEach(row => {
            const index = Number(row.dataset.index);
            const item = this.items[index];

            row.querySelector(".diet-start").addEventListener("change", e => {
                item.start = e.target.value;
                this.updateSummary();
            });

            row.querySelector(".diet-end").addEventListener("change", e => {
                item.end = e.target.value;
                this.updateSummary();
            });

            row.querySelector(".diet-percentage").addEventListener("input", e => {
                item.percentage = Number(e.target.value);
                this.updateSummary();
            });

            row.querySelector(".diet-interval").addEventListener("input", e => {
