/*EventTypes es un catálogo centralizado de eventos.*/

const EventTypes = {

    // PONDS
    POND_GEOMETRY_CREATED: "pond:geometryCreated",
    POND_CREATED: "pond:created",
    POND_SELECTED: "pond:selected",

    // MAP
    MAP_DRAW_START: "map:draw:start",
    MAP_DRAW_END: "map:draw:end",
    MAP_DRAW_POLYGON: "map:draw:polygon",
    MAP_ADD_POND: "map:addPond",
    MAP_CLEAR_TEMPORARY: "map:clearTemporary",

    // FEEDERS
    MAP_ADD_FEEDER: "map:addFeeder",
    MAP_PLACE_FEEDER: "map:placeFeeder",
    FEEDER_POSITION_SELECTED: "feeder:positionSelected",
    FEEDER_CREATED: "feeder:created",
    FEEDER_DELETED: "feeder:deleted",
    FEEDER_UPDATE: "feeder:update",
    FEEDER_SELECTED: "feeder:selected",
};

export default EventTypes;
